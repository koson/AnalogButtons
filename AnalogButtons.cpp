...
Button::Button(uint16_t value, void (*clickFunction)(void),
		void (*holdFunction)(void), uint16_t holdDuration,
...
AnalogButtons::AnalogButtons(uint8_t pin, uint8_t mode, uint16_t debounce,
		uint8_t margin)
{
	this->pin = pin;
...
void AnalogButtons::check()
{
	...
	if (millis() - time > ANALOGBUTTONS_SAMPLING_INTERVAL)
	{
		...
		{
			if ((int16_t) reading >= (int16_t) buttons[i].value - margin
					&& reading <= buttons[i].value + margin)
			{
